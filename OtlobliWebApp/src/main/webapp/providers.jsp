...
<%
       if(rememberUserName != "" || !rememberUserName.equals("")) {
            rememberMessage = "Welcome"+"     "+rememberUserName+"    "+"   to providers where you can manage providers";
        %>
        <br><br><br>
<h1 style="color:gold;"><%="Welcome" +"  "+  rememberUserName +"  where you can manage and view providers "%></h1>
...
%>
<br><br><br>
<h1 style="color:gold;">Welcome <%=userName%> where you can manage providers</h1>
...
<html>
  ...
  <%
   ...
   <jsp:include page="header_all.jsp" />
        ...
        <a href='add_provider.jsp' class="btn btn-primary btn-lg active" role="button" aria-pressed="true">Add New Provider</a>
        <h1>Providers List By JSP</h1>  

        <table class="table table-hover" border='1' width='100%'> 
            <tr><th>Id</th><th>Provider Arabic Name</th><th>Provider English Name</th>
                <th>Provider Hotline</th>
                <th>Provider Logo</th>
                <th>Edit</th>
                 <th>Edit provider image</th>
                <th>View provider branches</th>
                <th>View provider and Branches users </th>
                <th>View provider users only </th>
                <th>Delete</th>
            </tr>
            <%
                        for (ProviderBean providerBean : providers) {%>

            <tr><td> <%=providerBean.getId()%></td>
                <td><%=providerBean.getNameAr()%></td>
                <td><%=providerBean.getNameEn()%></td>
                <td><%=providerBean.getHotline()%></td>
                <td><img src="images/<%=providerBean.getLogoPathEn()%>" width="100" height="100"/></td>

                <td>
                    <a href="edit_provider.jsp?id=<%=providerBean.getId()%>"class="btn btn-primary btn-lg active" role="button" aria-pressed="true">Edit provider</a>

                </td>
                  <td>
                    <a href="edit_provider_image.jsp?id=<%=providerBean.getId()%>"class="btn btn-primary btn-lg active" role="button" aria-pressed="true">Edit Provider Image</a>

                </td>
                <td>
                    <a href="view_provider_branches.jsp?providerId=<%=providerBean.getId()%>"class="btn btn-primary btn-lg active" role="button" aria-pressed="true">View provider branches</a>

                </td> 
                <td>
                    <a href="view_provider_users.jsp?providerId=<%=providerBean.getId()%>"class="btn btn-primary btn-lg active" role="button" aria-pressed="true">View provider users and branches</a>

                </td> 
                <td>
                    <a href="view_providerusers_only.jsp?providers=<%=providerBean.getId()%>"class="btn btn-primary btn-lg active" role="button" aria-pressed="true">View provider users only</a>

                </td>
                 <td>
                    <a href="view_categories.jsp?providerId=<%=providerBean.getId()%>"class="btn btn-primary btn-lg active" role="button" aria-pressed="true">View Categories</a>

                </td> 
                <td>

                    <a href="DeleteProviderServlet?id=<%=providerBean.getId()%>" onclick="if (confirm('Are you sure you want to delete the record ?')) {
                                return true
                            } else {
                                return false
                            }" class="btn btn-danger btn-lg active" role="button" aria-pressed="true">Delete provider</a>
