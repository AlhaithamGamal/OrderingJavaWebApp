...
<%
       if(rememberUserName != "" || !rememberUserName.equals("")) {
        rememberMessage = "Welcome"+"     "+rememberUserName+"    "+"   to area branches where you can edit area branch ";

        %>
        <br><br><br>
<h1 style="color:gold;"><%="Welcome" +"  " + rememberUserName +"where you can manage and view area branches "%></h1>
...
%>
<br><br><br>
<h1 style="color:gold;">Welcome <%=userName%> where you can manage and view area branchess</h1>
...
<%
    ...
    String sid = request.getParameter("areaId");
   ...
   <jsp:include page="header_all.jsp" />
        ...
        <table class="table table-dark">
            <thead>
                <tr>
                    <th scope="col">ID</th>
                    <th scope="col">English(EN) Area Name</th>
                    <th scope="col">Arabic(AR)Area Name</th>
        ...
        <table  class="table table-hover" border='1' width='100%'>
            <tr>
                <th>ID</th>
                <th>Branch arabic Name(AR)</th>
                <th> Branch english Name(EN)</th>
                <th> provider arabic Name(EN)</th>
                <th> provider english Name(EN)</th>
                <th> Opened</th>
                <th> Closed </th>
                <th> phone </th>
                <th> Latitude</th>
            ...
            <% for (BranchBean branchBean : areaBean.getBranchList()) {%>
            <tr>
                <td> <%=branchBean.getId()%></td>
                <td> <%=branchBean.getNameAr()%></td>
                <td><%=branchBean.getNameEn()%></td>
                <td> <%=branchBean.getProviderBean().getNameAr()%></td>
                <td><%=branchBean.getProviderBean().getNameEn()%></td>
                ...
                <td> 
                    <a href="edit_branch_area.jsp?id=<%=branchBean.getId()%>&areaId=<%=id%>" class="btn btn-primary btn-lg active" role="button" aria-pressed="true">Edit Branch Details</a>

                </td>
                <td> 
                    <a href="view_delivery_area.jsp?branchId=<%=branchBean.getId()%>" class="btn btn-primary btn-lg active" role="button" aria-pressed="true">View delivery areas</a>
                </td>

                <td> 
                    <a href="DeleteBranchServlet?id=<%=branchBean.getId()%>&areaId=<%=id%>&flag=0" onclick="if (confirm('Are you sure you want to delete the record ?')) {
                                return true
                            } else {
                                return false
        ...
        <p><%=message%></p>
        <a href='add_branch.jsp?flag=0' class="btn btn-primary btn-lg active" role="button" aria-pressed="true">Add New Branch</a>
