 ...
 * @author HaithamGamal
 */
@WebServlet(name = "DeleteBranchServlet", urlPatterns = {"/DeleteBranchServlet"})
...
OtlobliBLGateway gateway =(OtlobliBLGateway)appCon.getBean("otlobliBLGateway");
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (flag != 1) {
                Integer areaId = Integer.parseInt(String.valueOf(request.getParameter("areaId")));
                int id = Integer.parseInt(request.getParameter("id")); //id of area recieved to return back
                gateway.deleteBranch(id);

                response.sendRedirect("view_area_branches.jsp?areaId=" + areaId);

            } else {
                Integer providerId = Integer.parseInt(request.getParameter("providerId"));
                int id = Integer.parseInt(request.getParameter("id")); //id of provider recieved to return back
                gateway.deleteBranch(id);
                response.sendRedirect("view_provider_branches.jsp?providerId=" + providerId);
            }
        } catch (Exception e) {
